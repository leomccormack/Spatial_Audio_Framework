...
/**
 * Available decoding (upper) orders.
 
 * @note The decoding order may be lower than the "master" order for any given
 *       frequency band, see ambi_dec_setDecOrder()
 */
typedef enum _AMBI_DEC_MASTER_ORDERS{
    MASTER_ORDER_FIRST = 1, /**< First-order decoding (4 channel input) */
 ...
 * at each order.
 */
typedef enum _AMBI_DEC_MIC_PRESETS{
    MIC_PRESET_IDEAL = 1,
    MIC_PRESET_ZYLIA,
    MIC_PRESET_EIGENMIKE32,
    MIC_PRESET_DTU_MIC
    
}AMBI_DEC_MIC_PRESETS;

/**
 * Available loudspeaker array presets
 */
typedef enum _AMBI_DEC_LOUDSPEAKER_ARRAY_PRESETS{
...
}AMBI_DEC_LOUDSPEAKER_ARRAY_PRESETS;
 ...
 */
typedef enum _AMBI_DEC_CH_ORDER {
 ...
 */
typedef enum _AMBI_DEC_NORM_TYPES {
 ...
 */
typedef enum _AMBI_DEC_CODEC_STATUS {
    ...
    CODEC_STATUS_INITIALISING     /**< Codec is currently being initialised,
                                   *   input audio should not be processed. */
} AMBI_DEC_CODEC_STATUS;

/** Maximum number of loudspeakers supported */
...
/** Length of string, returned by ambi_dec_getProgressBarText() */
 ...
 * @param[in] hAmbi ambi_dec handle
 * @param[in] Index For low (0) or high (1) frequency decoder
 * @param[in] See "DIFFUSE_FIELD_EQ_APPROACH" enum
 */
...
/**
 * Returns the master/maximum decoding order (see 'AMBI_DEC_MASTER_ORDERS' enum)
 */
...
/**
 * Returns the decoding order for the first band (see 'AMBI_DEC_MASTER_ORDERS'
 * enum)
 */
 ...
 * DIFFUSE_FIELD_EQ_APPROACH enum)
 *
 * @note It is suggested to preserve amplitude at low-frequencies and energy
 *       at high-frequencies
 *
 * @param[in] hAmbi ambi_dec handle
 * @param[in] index Index for low (0) or high (1) frequency decoder
 * @returns (see DIFFUSE_FIELD_EQ_APPROACH enum)
 */

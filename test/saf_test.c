...
void test__dvf_calcHighShelfParams(void){
    ...
    const int nTheta = 19;
    const int nRho = 5;
    const float rho[nRho] = {1.050000,1.250000,1.570000,2.381000,3.990000};
    const float g0_ref[nRho][nTheta] = { /* testRhoCoeffs_g_0 */
        {31.554217,20.659601,11.297794,7.400000,4.156557,1.615122,-0.492104,-2.265781,-3.768101,-6.186650,-6.133687,-7.051995,-7.823639,-8.466830,-8.958305,-9.349398,-9.591451,-9.801077,-9.816393},
    ...
    const float gInf_ref[nRho][nTheta] = { /* testRhoCoeffs_g_inf */
        {-5.347312,-4.707152,-7.500000,-6.023136,-6.907939,-8.190886,-8.708791,-10.210117,-11.350815,-11.991990,-15.263158,-12.426791,-11.148352,-11.331395,-10.812009,-10.362655,-10.111793,-9.926503,-10.019915},
    ...
    const float fc_ref[nRho][nTheta] = { /* testRhoCoeffs_f_c */
       /* TODO: debug negative freq: -2427.961709 */  {444.649733,454.662130,492.799208,-2427.961709,3161.462192,3394.479764,3638.570889,4210.955751,4448.408885,4598.132076,4217.808050,4889.091478,4877.848121,5002.827951,2173.704998,5195.561392,5221.016169,5178.768363,5743.918118},
...
//            TEST_ASSERT_FLOAT_WITHIN(acceptedTolerance, gInf_ref[ri][ti], gInf);
            TEST_ASSERT_FLOAT_WITHIN(acceptedTolerance, fc_ref[ri][ti], fc);
        }
    }
}
...
void test__dvf_calcHighShelfCoeffs(void){
    ...
    const float b0_ref[nRho][nTheta] = { /* testIIRCoefs_b0 */
        {20.726250,15.794291,0.589641,0.111472,0.127698,0.128511},
    ...
    const float b1_ref[nRho][nTheta] = { /* testIIRCoefs_b1 */
        {-19.453106,-14.819228,-0.359521,-0.058195,-0.056312,-0.050765},
    ...
    const float a1_ref[nRho][nTheta] = { /* testIIRCoefs_a1 */
        {-0.966336,-0.965596,-0.821813,-0.891957,-0.781386,-0.759290},
    ...
    /* IIR coeffs to be tested */
    float b0, b1, a1;
    
    for(int ri = 0; ri < nRho; ri++){
        float r = rho[ri];
        for(int ti = 0; ti < nTheta; ti++){
            calcHighShelfCoeffs(theta[ti], r, fs, &b0, &b1, &a1);

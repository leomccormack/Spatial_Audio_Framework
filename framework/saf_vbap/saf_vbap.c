...
)
{
    ...
    if(enableDummies){
        ...
        if(needDummy[0] || needDummy[1]){
            ...
            ls_dirs_d_deg = malloc(L_d*2*sizeof(float));
...
)
{
    ...
    azi = malloc(N_azi * sizeof(float));
    ele = malloc(N_ele * sizeof(float));
    for(fi = -180.0f, i = 0; i<N_azi; fi+=(float)az_res_deg, i++)
        azi[i] = fi;
    for(fi = -90.0f,  i = 0; i<N_ele; fi+=(float)el_res_deg, i++)
        ele[i] = fi;
    src_dirs = malloc((N_azi*N_ele)*2*sizeof(float));
    for(i = 0; i<N_ele; i++){
        for(j=0; j<N_azi; j++){
...
)
{
    ...
    (*vbap_gtableComp) = calloc(nTable*3,sizeof(float));
    (*vbap_gtableIdx) = calloc(nTable*3,sizeof(int));
...
)
{
    ...
    gains_sum = calloc(nTable,sizeof(float));
    for(i=0; i<nTable; i++)
        for(j=0; j<nDirs; j++)
...
)
{
    ...
    ls_vertices = malloc(L*2*sizeof(float));
    for(i=0; i<L; i++){
...
)
{
    ...
    N_azi = (int)((360.0f/(float)az_res_deg) + 1.5f);
    src_dirs = malloc(N_azi * sizeof(float));
    for(fi = -180.0f, i = 0; i<N_azi; fi+=(float)az_res_deg, i++)
        src_dirs[i] = fi;
    out_pairs = NULL;
    findLsPairs(ls_dirs_deg, L, &out_pairs, &numOutPairs);

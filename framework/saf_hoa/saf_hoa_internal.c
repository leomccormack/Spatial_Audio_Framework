...
)
{
    ...
    Yna_W = malloc(nSH * N_dirs*sizeof(float_complex));
    Yna_W_Yna = malloc(nSH * nSH * sizeof(float_complex));
    Yna_W_H = malloc(nSH * 2 * sizeof(float_complex));
    B = malloc(nSH * 2 * sizeof(float_complex));
...
)
{
    ...
    Yna_W_Yna = malloc(nSH * nSH * sizeof(float_complex));
    Yna_W_H = malloc(nSH * 2 * sizeof(float_complex));
    B_ls = malloc(nSH * 2 * sizeof(float_complex));
    hrtfs_ls = malloc(2*N_dirs*sizeof(float_complex));
...
)
{
    ...
    /* integration weights */
    W = calloc(N_dirs*N_dirs, sizeof(float));
    if(weights!=NULL)
        for(i=0; i<N_dirs; i++)
    ...
    hrtf_dirs_rad = malloc(N_dirs*2*sizeof(float));
    cnd_num = malloc((Nh_max+1)*sizeof(float));
    ...
    Y_td_cmplx = malloc(nSH_nh*K_td*sizeof(float_complex));
    for(i=0; i<nSH_nh*K_td; i++)
        Y_td_cmplx[i] = cmplxf(Y_td[i], 0.0f);
    
    /* calculate decoding matrix per band */
    Ynh_Ytd = malloc(N_dirs * K_td * sizeof(float));
    tmp = malloc(N_dirs * K_td * sizeof(float));
    W_Ynh_Ytd = malloc(N_dirs * K_td * sizeof(float_complex));
    hrtfs_td = malloc(2*K_td*sizeof(float_complex));
    B = malloc(nSH * 2 * sizeof(float_complex));
    for(band=0; band<N_bands; band++){
...
)
{
    ...
    Yna_W_H = malloc(nSH * 2 * sizeof(float_complex));
    B = malloc(nSH * 2 * sizeof(float_complex));
    hrtfs_mod = malloc(2*N_dirs*sizeof(float_complex));
    cblas_cgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, nSH, N_dirs, N_dirs, &calpha,
                Y_na, N_dirs,
                W, N_dirs, &cbeta,
                Yna_W, N_dirs);
...
)
{
    ...
    Yna_W_Yna = malloc(nSH * nSH * sizeof(float_complex));
    Yna_W_H = malloc(nSH * 2 * sizeof(float_complex));
    B_magls = malloc(nSH * 2 * sizeof(float_complex));
    hrtfs_ls = malloc(2*N_dirs*sizeof(float_complex));
    H_mod = malloc(2*N_dirs*sizeof(float_complex));
    cblas_cgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, nSH, N_dirs, N_dirs, &calpha,
                Y_na, N_dirs,
                W, N_dirs, &cbeta,
                Yna_W, N_dirs);

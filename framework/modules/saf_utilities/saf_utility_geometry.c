...
{
    int i;
    if(dirsIncl!=dirsElev)
        cblas_scopy(nDirs*2, dirsElev, 1, dirsIncl, 1);
    if(degreesFlag){
        for (i=0; i<nDirs; i++)
...
)
{
    int i;
    if(dirsIncl!=dirsElev)
        cblas_scopy(nDirs*2, dirsIncl, 1, dirsElev, 1);
    if(degreesFlag){
        for (i=0; i<nDirs; i++)
...
{
#if (defined(SAF_USE_APPLE_ACCELERATE_LP64) || defined(SAF_USE_APPLE_ACCELERATE_ILP64)) && 0
    float sqdist;

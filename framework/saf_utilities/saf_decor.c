...
(
    ...
    int* delayTF    /* the resulting time delays per channle and frequency; nFreq x nChannels */
)
{
    int band, ch;
    int* randperm_nCH;
    ...
    randperm_nCH = malloc(nChannels*sizeof(int));
    delayRangeMax = malloc(nFreqs*sizeof(float));
    delayRangeMin = malloc(nFreqs*sizeof(float));
    tmp_delays = malloc(nChannels*sizeof(float));
    delays = malloc(nFreqs*nChannels*sizeof(float));

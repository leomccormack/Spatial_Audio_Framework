...
{
    *phMC = malloc(sizeof(safMatConv_data));
    ...
    h->ovrlpAddBuffer = malloc(nCHout*(h->length_h_pad)*sizeof(float));
    h->x_pad = calloc((h->nCHin)*(h->length_h_pad), sizeof(float)); // CALLOC
    h->y_pad = malloc((h->nCHout)*(h->length_h_pad)*sizeof(float));
    h->hx_n = malloc((h->length_h_pad) * sizeof(float));
    h->z_n = malloc((h->length_h_pad) * sizeof(float));
    h->H_f = malloc((h->nCHout)*(h->nCHin)*(h->length_h_fft)*sizeof(float_complex));
    h->X_n = malloc((h->nCHin)*(h->length_h_fft)*sizeof(float_complex));
    h->HX_n = malloc((h->length_h_fft)*sizeof(float_complex));
    safFFT_create(&(h->hFFT), h->length_h_pad);
    h_pad = calloc(h->length_h_pad, sizeof(float));
    for(i=0; i<nCHout; i++){
        for(j=0; j<nCHin; j++){
